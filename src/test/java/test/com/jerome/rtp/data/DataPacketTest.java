...
public class DataPacketTest {
    ...
    public void testEncode() {
        ...
        for(int i=0;i<ALAW_RTP_PACKET_SAMPLE.length;i++){
        	System.out.println("i = "+i+" : "+(ALAW_RTP_PACKET_SAMPLE[i]==array[i]));
        }
    }
    ...
    public void testEncodeDecodeWithFixedBlockSize() {
        ...
        ByteBufferManager mm = new ByteBufferManager();
        Buffer encoded = packet.encode(64,mm);
        
        encoded.position(0);
        
        System.out.println(ByteUtils.writeArrayAsHex(encoded.array(), true));
        assertEquals(64, BufferUtils.readableBytes(encoded));
